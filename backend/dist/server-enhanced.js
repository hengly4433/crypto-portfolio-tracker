...
const job_manager_featured_1 = require("./infrastructure/jobs/job-manager-featured");
...
async function checkDependencies() {
    ...
    if (features_1.FEATURES.ENABLE_PRICE_APIS) {
    ...
    if (features_1.FEATURES.ENABLE_EXCHANGE_SYNC) {
...
function displayStartupBanner() {
    ...
    const config = (0, features_1.getConfigSummary)();
    console.log(`Mode: ${config.mode}`);
    console.log(`Port: ${PORT}`);
    console.log('\n📊 Features:');
    console.log(`  • Background Jobs: ${features_1.FEATURES.ENABLE_BACKGROUND_JOBS ? '✅' : '❌'}`);
    console.log(`  • Price APIs: ${features_1.FEATURES.ENABLE_PRICE_APIS ? '✅' : '❌'}`);
    console.log(`  • Exchange Sync: ${features_1.FEATURES.ENABLE_EXCHANGE_SYNC ? '✅' : '❌'}`);
    console.log(`  • Candle Charts: ${features_1.FEATURES.ENABLE_CANDLE_AGGREGATION ? '✅' : '❌'}`);
    console.log(`  • Email Notifications: ${features_1.FEATURES.ENABLE_EMAIL_NOTIFICATIONS ? '✅' : '❌'}`);
    console.log(`  • Telegram Notifications: ${features_1.FEATURES.ENABLE_TELEGRAM_NOTIFICATIONS ? '✅' : '❌'}`);
    console.log(`  • Audit Logging: ${features_1.FEATURES.ENABLE_AUDIT_LOGGING ? '✅' : '❌'}`);
    console.log('\n🔧 Requirements:');
...
async function main() {
    try {
        ...
        const preset = process.env.CONFIG_PRESET || 'DEVELOPMENT';
        if (process.env.CONFIG_PRESET) {
            (0, features_1.applyPreset)(preset);
        }
        ...
        console.log('\n⚙️  Initializing jobs...');
        try {
            (0, job_manager_featured_1.initializeAllJobs)();
        }
        catch (jobError) {
            console.warn(`Job initialization warning: ${jobError.message}`);
