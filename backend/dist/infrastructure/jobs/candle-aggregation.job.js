...
exports.candleQueueEvents = exports.candleAggregationWorker = exports.candleAggregationQueue = void 0;
exports.scheduleCandleAggregation = scheduleCandleAggregation;
exports.initializeCandleAggregationJobs = initializeCandleAggregationJobs;
...
exports.candleAggregationQueue = new bullmq_1.Queue('candle-aggregation', { connection: (0, redis_1.getRedisConnection)() });
// Worker to process candle aggregation jobs
exports.candleAggregationWorker = new bullmq_1.Worker('candle-aggregation', async (job) => {
...
exports.candleQueueEvents = new bullmq_1.QueueEvents('candle-aggregation', { connection: (0, redis_1.getRedisConnection)() });
/**
 ...
 */
async function aggregateCandlesForTimeframe(timeframe) {
 ...
 */
async function aggregateCandlesForAsset(assetId, timeframe) {
    ...
    const latestCandle = await db_1.prisma.candle.findFirst({
    ...
    const priceSpots = await db_1.prisma.priceSpot.findMany({
        where: {
            assetId,
            fetchedAt: {
                gte: startTime,
    ...
    for (const candle of candles) {
        await db_1.prisma.candle.upsert({
            where: {
 ...
 */
function groupPriceSpotsIntoCandles(priceSpots, timeframe) {
    const candles = [];
    ...
    for (const priceSpot of priceSpots) {
        ...
        if (!currentCandle || currentCandle.openTime.getTime() !== candleStartTime.getTime()) {
            ...
            currentCandle = {
                ...
                volume: new client_1.Prisma.Decimal(0), // Volume not available from price spots
            };
        }
        else {
 ...
 */
function getCandleEndTime(startTime, timeframe) {
 ...
 */
function scheduleCandleAggregation() {
    // Minute aggregation: every minute
    node_cron_1.default.schedule('* * * * *', async () => {
        console.log('Scheduling M1 candle aggregation...');
        await exports.candleAggregationQueue.add('aggregate-m1', { timeframe: 'M1' }, {
    ...
    node_cron_1.default.schedule('*/5 * * * *', async () => {
        ...
        await exports.candleAggregationQueue.add('aggregate-m5', { timeframe: 'M5' }, {
    ...
    node_cron_1.default.schedule('0 * * * *', async () => {
        ...
        await exports.candleAggregationQueue.add('aggregate-h1', { timeframe: 'H1' }, {
    ...
    node_cron_1.default.schedule('5 0 * * *', async () => {
        ...
        await exports.candleAggregationQueue.add('aggregate-d1', { timeframe: 'D1' }, {
    ...
    node_cron_1.default.schedule('0 1 * * *', async () => {
        ...
        const result = await db_1.prisma.priceSpot.deleteMany({
            where: {
                fetchedAt: {
                    lt: cutoffDate,
                },
 ...
 */
function initializeCandleAggregationJobs() {
    scheduleCandleAggregation();
    // Listen for worker events
    exports.candleAggregationWorker.on('completed', (job) => {
        console.log(`Candle aggregation job ${job.id} completed`);
    });
    exports.candleAggregationWorker.on('failed', (job, err) => {
