...
export class OkxClient implements ExchangeClient {
  ...
  constructor(apiKey: string, apiSecret: string, passphrase?: string, isTestnet: boolean = false) {
    this.apiKey = apiKey;
  ...
  async getBalances(): Promise<ExchangeBalance[]> {
    ...
    try {
      ...
      })).filter((balance: ExchangeBalance) => balance.total > 0);
    } catch (error) {
  ...
  async testConnection(): Promise<boolean> {
    ...
    try {
      // Make a simple public API call to test connection
      interface OkxPublicResponse {
        code: string;
        data: any[];
        msg: string;
      }
      
      const response = await axios.get<OkxPublicResponse>(`${this.baseUrl}/public/time`);
   ...
   */
  static mapSymbolToAsset(okxSymbol: string): { base: string; quote: string } | null {
